...
resource "aws_lambda_function" "lambda" {
  #filename         = "modules/ecs-cluster/ecs_task_drainer.zip"
  filename         = "${path.module}/lambdas/ec2_ecs_drainer.zip"
  #function_name    = "lambda-handler-TEST"
  function_name    = "lambda-handler-${var.ecs_cluster_name}"
  role             = "${var.lambda_iam_role}"
  #handler          = "index.lambda_handler"
  handler          = "ec2_ecs_drainer.lambda_handler"
  runtime          = "python2.7"
  #source_code_hash = "${base64sha256(file("modules/ecs-cluster/ecs_task_drainer.zip"))}"
